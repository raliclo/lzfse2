...
| ---: | --- | ---: | ---: | ---: | --- |
| 1 | 自己的過濾器把失敗藏起來 | **8** | 4 | **4** | `helper/run_checked.zsh` |
...
## 1. 自己的過濾器把失敗藏起來 — **已發生 8 次（2026-08-27 至 09-05）**

**症狀**：指令「成功」了，但實際上什麼都沒做；或測試「通過」了，而摘要行下面就是失敗。

**八次的實際形式**（第 5、6、7 次在下方單獨說明，都發生在為了防這條而做的事情裡；
第 8 次則是為了查證另一份回報而下的比對指令，見本節末）：
...
**第 8 次——2026-09-05 同日，用截斷過的輸出比對兩個檔案，然後宣告整行相同。** 一個
subagent 回報 `OPTIMIZATION-en.md:4269` 的數字損毀。我去查證：

```zsh
printf 'ZH: '; sed -n "4269p" OPTIMIZATION.md    | cut -c1-110
printf 'EN: '; sed -n "4269p" OPTIMIZATION-en.md | cut -c1-110
```

兩邊印出的前綴一致，於是我**推翻了那份回報**，並把「複驗值得做」當成教訓寫給使用者。
實際上該列有 10 格，損毀的是**第 8 格**（`1566.2 MB` 被寫成 `156.2 MB`），而 `cut -c1-110`
在第 7 格就截斷了。我比對的是「被截斷後的前綴相等」，卻宣告了「整行相同」。

三件事讓它更糟。其一，**`cut` 正是本條所講的過濾器**——為了讓輸出好讀而寫，樣式只涵蓋
預期中的部分，非預期者消失，而消失與「沒問題」在畫面上相同。其二，那個錯誤的數字有內在
的荒謬：`156.2` 是「最高 Encode RSS」，比同列「最低」的 `1339.4` 還小，**看完整行光憑
這一點就會發現**。其三，我當時另有一個未經檢查的前提——兩個檔案逐行對齊；那天成立，兩天
後中文版長了 39 行就不再成立，`ZH:4269` 已變成空行。

**現在怎麼防**：比對兩個檔案的某一行時，不要用 `cut`／`head -c` 截斷後目視。用會看完整行
的工具——`cmp`、`diff <(sed -n "${a}p" A) <(sed -n "${b}p" B)`——讓「相同」由工具判定而
不是由我判定。要縮短輸出可以，但那時就**不能**下「相同」的結論。另外，跨檔案的行號對應
不可假設，先以內容錨定（`grep -n` 該列的標籤）再取行。
