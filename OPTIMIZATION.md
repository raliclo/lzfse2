...
**其二，管線平行。** external 的 `zstd -d -c | tar -xf -` 是兩個行程同時工作——zstd
解壓的同時 tar 已在寫檔；native 是單一行程，兩件事串在同一條時間線上。寫檔增量
0.25 對 0.57 秒即為此。`FileWriterPool` 平行的是「寫檔彼此之間」，不是「解壓對寫檔」。
...
1. **修 `run_round.command` 的 `rc=$?` 位置**（承 R49-Mac 待辦第 1 條，仍未修）。把它
   ...
   構成成功的證據。
2. **讓解壓與寫檔重疊**（第 3 節）。目前解壓執行緒解完一個 chunk 才寫一個，而 external
   的兩行程管線正是靠這一點取得 0.25 對 0.57 秒的優勢。方向是讓解壓執行緒把 chunk 交給
   `FileWriterPool`，而非自行寫出。這是本輪找到的最明確的改善點。
