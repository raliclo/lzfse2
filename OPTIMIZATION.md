...
**結果**：`DIFF_EXIT:0` — output-identical，零差異。
...
> R49-Mac 記錄的解壓回歸，並取得修復後的第一份乾淨基準。
>
> **成立的結論只有一項：回歸確實修好了。** 除此之外，R50 與 R48 之間的所有差異經查證
> 皆為量測雜訊——包含看似「變快」的那些。詳見第 2 節。

執行 2026-08-28 13:05:45 → 22:48:02，共 **9 小時 42 分**。機器 `Mac16,10`，
`os_version` `27.0 (26A5421a)`，swift_tar `7e64057`。

判定（`helper/status_monitor.zsh --verdict`）：失敗訊號 **0**、解壓一致性比對
**48 OK / 0 FAILED**、八個必要步驟標記全部到齊。**未採用 exit code**——原因見第 4 節。

## 1. `aea0427` 的回歸已修復

解壓耗時（秒，n=40）：

| 資料集 | 格式 | R48 | R49（含回歸）| **R50** |
| --- | --- | ---: | ---: | ---: |
...
**12 秒級全部回到 3 秒級，九個格式無一例外。** `claw-code` 同樣：R49 的 2.5–3.3 秒回到
1.4–2.4 秒。

這種一致性正是本節與第 2 節的分野：回歸是**數量級**的差異且方向一致，而雜訊有正有負、
在兩個資料集上甚至相反。

## 2. 其餘差異皆為雜訊，包含看似變快的那些

R50 對 R48 的 decode 差異有正有負，範圍 −28% 到 +36%。**三項獨立證據顯示它們不是真的。**

**方向矛盾。** 同一格式、同一版本，在兩個資料集上結論相反：
...
**E-Cluster 不能解釋。** R48-Mac 第 2 節曾以 E-Cluster 參與度解釋輪間差異，本輪查證後
該關聯不成立：

| 格式 | R48 E-Cluster | R50 E-Cluster | ΔE | decode Δ |
| --- | ---: | ---: | ---: | ---: |
| optimal3 | 75.82% | 92.47% | +16.7 | +1.4% |
| other3 | 77.32% | 86.05% | +8.7 | **+22.4%** |

E-Cluster 上升較少的 other3 反而慢得多。若該關聯成立，方向應相反。

**直接複量推翻兩邊。** 以同一份封存、6 輪交錯取最小值：

| 格式 | R48 | R50 | 複量 | 判讀 |
| --- | ---: | ---: | ---: | --- |
| Apple | 1.78 | 1.59 | **1.51** | R50 的「快 10.7%」不成立 |
| Other3 | 1.34 | 1.64 | **1.32** | R50 的「慢 22.4%」不成立，複量回到 R48 |

**因此 R50 的 decode 各列只能用來確認回歸已修，不得用來宣稱任何格式變快或變慢。**

本輪之前已有四次同類事件被複量推翻（BVX3 encode +81%、claw decode +21%、`--exclude`
+9.5%、加密建立 +13.6%）。這台機器單次量測的變異可達 2 倍以上，**交錯多輪取最小值是
唯一可信的做法**；次數不足時離群值會主導結論。

## 3. native 對 external zstd

R49-Mac 是 macOS 首次量到 native libzstd，但當時無從與 external 正面比較。本節以同一份
`claw-code`（1,351 MB、5,402 檔）、同為 level 9、6 輪交錯取最小值直接對照。

| | external（`zstd` CLI）| native（swift_tar 內建）| |
| --- | ---: | ---: | --- |
| encode | 5.71 秒（237 MB/s）| **2.65 秒（510 MB/s）** | native **快 2.2 倍** |
| decode | **1.45 秒（932 MB/s）** | 1.88 秒（719 MB/s）| native 慢 30% |
| 封存大小 | **365.8 MB** | 382.6 MB | native 大 4.6% |
...
| | external | native | |
| --- | ---: | ---: | --- |
| 純解壓縮（不解析 tar、不寫檔）| 0.91 | 1.15 | +26.2% |
| 完整解出 | 1.16 | 1.72 | +48.3% |
| 寫檔的增量 | **0.25** | **0.57** | |

**其一，分塊。** `TAR_CHUNK_SIZE = 1 << 22`（4 MiB），每個 chunk 壓成獨立的 zstd frame：
...
336 個 frame 各自從零開始，跨 chunk 的重複抓不到——這同時解釋了 +26% 的純解壓縮成本
與 +4.6% 的壓縮比損失。

**這是設計取捨而非缺陷**：分塊正是 `ParallelChunkSink` 能把工作分給多核的前提，而
encode 快 2.2 倍就是它換來的。

**其二，先前寫的是「管線平行」——那個結論已被推翻，記述如下。**

原本的推論是：external 的 `zstd -d -c | tar -xf -` 兩個行程同時工作，native 是單一行程
串在同一條時間線上，故差距來自平行度。**它是只看 `real` 寫出來的。**

加入 `user` 與 `sys` 之後，該推論不成立：

| | real | user | sys | CPU/wall |
| --- | ---: | ---: | ---: | ---: |
| native 解出 | 1.61 | 0.97 | 1.73 | 1.68 |
| external 解出 | 2.27 | 1.10 | 2.14 | 1.43 |

**`user` 幾乎相同**（0.97 對 1.10）——兩邊的計算量一樣，分塊沒有讓 libzstd 多做工。而
CPU/wall 是 native **較高**（1.68 對 1.43），與「native 較不平行」正好相反。

而且這張表本身也翻轉了勝負：**在 RAM disk 上 native 解出反而較快**（1.61 對 2.27）。先前
量到 external 較快（1.45 對 1.88）是在內接碟上——所以那個差距屬於儲存層，不屬於 codec。

### sys 時間的去向（目前僅有的線索）

以 `time -l` 的計數（同一份 claw-code，解出至 RAM disk）：

| | sys 秒 | page faults | vol ctx-sw | invol ctx-sw |
| --- | ---: | ---: | ---: | ---: |
| native 解出 | 1.71 | **128,042** | 90,600 | 10,524 |
| external 解出 | 2.13 | 1,990 | 119,727 | 9,666 |

**page fault 差 64 倍。** 那指向 `FileWriterPool` 逐檔緩衝 4 MiB（`smallFileMax`）的代價。
需留意 external 是兩個行程而 `time -l` 只涵蓋外層 `sh`，故其計數偏低——此處比的是形狀，
不是絕對值。

**這仍不是結論，是下一步的起點。** 本節的頭兩版結論各自被下一組量測推翻一次，兩次的成因
相同：在數字不足時就提出了機制。第三次不重蹈，先把數字補齊。

量測工具已固化為 [`swift_tar/verifications/zstd_decode_gap.zsh`](swift_tar/verifications/zstd_decode_gap.zsh)，
五個模式各對應一項曾經推翻結論的證據：`pipeline`（real/user/sys 拆解）、`threads`（`-n` 掃描）、
`syscall`（page fault 與 context switch）、`chunk`（frame 數）、`storage`（RAM disk 對實際磁碟）。

### `-n` 對 zstd 解壓沒有可調空間（已排除）

既然分塊是為了平行，那 `-n`（in-flight chunk 數）是否該為 zstd 解壓另行調整？**不必——
怎麼設都一樣。**

在 RAM disk 上解出同一份 `claw-code` 封存（3 GB HFS ram://，消除磁碟飽和），5 輪交錯：
...
全部落在 1.61–1.66 秒。`n4` 的 −1.6% 與 `n40` 的 +1.8% 幅度相同、方向相反，是雜訊的
兩端而非「4 核最佳」。純解壓縮（`--cat`，完全不寫檔）更明確：六個 `-n` 值皆為
1.14–1.15 秒，範圍 ±0.8%。

成因清楚：`-n` 控制的是 `ParallelChunkSink` 的 in-flight chunk 數，那是**壓縮端**的機制。
解壓端的 336 個 frame 必須按序還原成連續的 tar 串流，順序是硬性約束，沒有可平行的空間。

**這組量測本身也是一次教訓。** 同樣的掃描先在內接碟上做，得到「`-n 1` 1.78 秒、其餘
3.25–5.33 秒」，看起來像 `-n 1` 大勝。交錯複驗揭穿了它：
...
每組的第一次都快、其後全部劣化——決定數字的是「排在序列第幾位」，因為反覆 `rm -rf`
再寫入 1.3 GB 把 SSD 的寫入路徑塞滿了。**最小值在此失效**：它挑到的是序列早期的那次。
改用 RAM disk 之後逐次數值才穩定下來（1.61–1.92）。

於是本節排除了第二個假設（第一個是第 2 節的 E-Cluster），**待辦第 2 項成為唯一剩下的
明確方向**：RAM disk 上的 1.64 秒對內接碟上的 1.72–1.88 秒，差距正是寫檔；而 external
的優勢（寫檔增量 0.25 對 0.57 秒）也在同一處。
...
R49 與 R50 完全相同，R48 不同——這證實差異來自實作替換（分塊 vs 連續串流）而非任何
回歸，且 native 的輸出是確定性的。

## 4. 判定不採用 exit code，以及 status_monitor.zsh 的兩個缺陷

本輪的判定依 `helper/status_monitor.zsh --verdict`，理由記於 R49-Mac 第 2 節：
`run_round.command` 的 `rc=$?` 位在 `rm -f .bench_env` 之後，而 `rm -f` 永遠回 0，故
`BENCH_DONE` 無條件寫出、`BENCH_FAILED` 永不執行、整輪永遠 exit 0。**該缺陷仍未修。**

而該腳本本身在首次實戰中暴露兩個缺陷，皆已修正：

**判定把乾淨的一輪報成失敗。** `grep -c` 在零命中時印出 `0` 但退出碼是 1，原本寫的
`|| print 0` 於是補印一次，變數成為 `"0\n0"`，其後的算術以 bad math expression 失敗，
判定落到「未完成」那一支。**這正是該腳本要防的錯誤，只是方向相反**——不是把失敗說成
成功，而是把成功說成失敗。兩者都讓判定失去意義。

**過濾器吃掉六個資料集的完成通知。** 原本的 `DONE [0-9]` 要求 `DONE` 後接時間戳，但
`LZ4BENCH_DONE claw-code-n40 13:29:16` 後面接的是名稱。結果整整 9 小時未收到任何進度，
而那看起來就像「一切正常地安靜」——正是該腳本開頭寫明要避免的那種沉默。改為
`_DONE( |$)`。

## 5. 本輪資料

6 次 `-n` 掃描（claw-code 與 llama.cpp 各 n=40／8／4）、54 筆結果、power 欄位 54/54
皆有讀數、48 次解壓一致性比對全數通過、trace 與 CPU call tree 分析各一輪。
...
2. ✅ **已量（2026-09-04）：`smallFileMax` 被排除，候選數回到零。**（第 3 節）

   量法不需要重建。`swift_tar.swift:4019` 那一行本身就是開關：≤4 MiB 的成員整份緩衝進一個
   `Data()`，>4 MiB 的完全跳過而 inline 串流。**只要讓成員大小跨過 4 MiB，就能在同一個
   執行檔上開關這條路徑。** 五個語料、7 輪交錯、取最小值、RAM disk，輪間變異 < 1%：

   | 語料 | 成員 | 路徑 | fault | 每 MiB | 每 4KiB 分頁 |
   | --- | --- | --- | ---: | ---: | ---: |
   ...
   **決定性的是 D 與 E**：成員數相同、大小相鄰（4 對 5 MiB）、分支翻面。每頁 fault 由 1.07
   平順走到 0.91，**分界處沒有不連續**。若逐檔緩衝是成因，D 應遠高於 E。

   **更強的反證是 C**：它把緩衝路徑用得最重（96 個成員全部走它），fault 卻最少——每 MiB 84，
   是 A 的三分之一。

   每頁 fault 隨單次配置的大小連續變化、跨越分支不跳動，形狀指向配置器而非寫入池：macOS 的
   malloc 對超過門檻的配置以 mmap 取得、釋放時歸還，下一次配置於首次觸碰時逐頁 zero-fill。
   inline 路徑同樣逐塊配置，同樣付這個代價。

   量測與完整輸出：`swift_tar/verifications/page_fault_attribution.zsh`（`.txt` 為保留的執行
   結果）。**界限**：該實驗量的是 swift_tar 在合成、不可壓縮語料上的解壓，**不是** native 對
   external 那個 64 倍差距本身；它只回答「逐檔緩衝是不是 fault 的來源」，答案是否。

   **於是四個假設全部排除**（E-Cluster 參與度、`-n` 設定、平行度、`smallFileMax`），而
   **本節沒有剩下任何候選**。下一步不該是再提一個機制——那已經被推翻三次了——而是取得逐符號
   的 profile（`sample` 或 Instruments 的 Allocations），讓資料指出配置點，而不是由推理指定。

   > **後續（2026-09-06）：那個 profile 做了，而它先推翻了本項的前提。**
   >
   > swift_tar `9aa0cf6` 在動 profiler 之前先重測現象本身，同一支腳本、三種形狀、兩個平台：
   >
   ...
   >
   > **那個差距是 Linux 專有的，而 macOS 上 swift_tar 現在是較快的一方**，語料愈碎領先愈多。
   >
   > **因此本項上方的量測仍然成立，但它回答的問題已經不是最初那一個。** 它證明的是
   > 「逐檔緩衝不是 page fault 的來源」——那在 macOS 上仍為真，且與平台無關地排除了一個機制。
   > 它**不**證明「native 在 macOS 上較慢」的成因，因為那個前提本身已不成立。上方表格請只
   > 引用為前者。
   >
   > `58d1eb0` 續以 LLVM PGO 逐函式計數（選計數而非取樣，因為計數與機器負載無關；當時 host
   > load 4.63，其中 74.7% 是另一個專案的模擬器）。結論同樣是否定的：**時間不在 swift_tar
   > 自己的 Swift 程式碼裡**——解開 160 MB 的全部儀器化區塊執行次數合計 21,743，`readExactly`
   > 在 20 個成員、160 MB 之中只跑了 63 次。
   >
   > **方法上值得留下的一句是 `9aa0cf6` 的：先量前提，而不是先 profile。** profiler 一定會
   > 回報「某個最花時間的函式」，而那個函式接著就會被當成成因——即使那個現象已經不存在。

   **本節的前兩版結論都是在數字不足時提出機制而被推翻的**：先是「解壓與寫檔沒有重疊」
   （`user` 相同、CPU/wall 反而 native 較高，推翻），再是「external 靠管線平行取勝」
   （RAM disk 上 native 反而較快，推翻）。三個競爭假設已排除：E-Cluster 參與度（第 2 節，
   相關性方向相反）、`-n` 的設定（第 3 節，六個值皆在 ±1.8% 內）、平行度（本節，CPU/wall
   方向相反）。
...
| 項目 | 說明 |
| --- | --- |
| **swift_tar combined flags** | swift_tar 原本不支援 `-czf`、`czf`（no-dash POSIX 形式）等 combined short flags，導致 benchmark 全程失敗。修正：在 `main()` 入口展開 combined flags，實現與系統 tar 的完整相容。 |
| **run_round.command -swift_tar** | 新增 opt-in 旗標；帶旗標時建立 PATH shim（`tar → swift_tar`），並以 `sudo --preserve-env=PATH` 讓子行程繼承修改後的 PATH；不帶旗標維持原行為。 |
| **NGResult enum（lzfse-cli.swift）** | `nextGroup()` 原以 captured `var misaligned` 回傳截斷串流錯誤；Swift `-O` SSA 分析追蹤 captured bool，判定 `if misaligned` 在所有可達路徑恆為 false → warning: will never be executed。修正：讓 `nextGroup()` 直接回傳 `NGResult`（`.group` / `.eof` / `.misaligned`），消除 side-channel captured variable，呼叫端改用 `innerLoop: switch`。 |
| **lzfse2 submodule 登記** | 將 swift_tar 以 git submodule 方式（`git@github.com:raliclo/swift_tar.git`）登記至 lzfse2，不再以目錄直接內嵌。 |

## 驗證

- `TEST_OK`（`lzfse -test` 全數通過，含 NGResult 重構後的往返驗證）。
- 全 R43-Mac round 跑畢：`BENCH_DONE 17:38:38`，零失敗。
- swift_tar decode 管線（`lzfse -decode -so | tar -xf -`）對兩資料集所有格式均通過 `[Success] 解壓內容與 tgz 一致` 比對。
...
> **與 R42-Mac 比較**：encode 與 decode 速度均有顯著提升，主因是 swift_tar 多核心並行取代系統 tar，詳見下方對照表。部分格式的壓縮比（normalized to TGZ）也略有改善，推測是 swift_tar 產生略不同的 tar byte stream（header/padding 差異改變了 LZ window 的重複區段分佈）所致。

## 2. Encode 速度對照：R42-Mac vs R43-Mac（n=40）

> swift_tar 的加速效果依演算法而異：**I/O 密集型**（TGZ / Other3 / BVX3 / TLZ4 / ZSTD）因 tar 讀取管線加速而大幅受益；**CPU 密集型**（Optimal3 / Lazy2 / Optimal）壓縮本身是瓶頸，加速幅度有限。

### claw-code（n=40）

| 格式 | R42 Enc MB/s | R43 Enc MB/s | 變化| 主因|
| --- | ---: | ---: | ---: | --- |
| TGZ | 47.56 | 310.83 | **+554%** | swift_tar 多核心 gzip（vs 系統 tar 單核） |
| Other3 | 339.49 | 550.38 | +62% | tar 輸入管線加速，encode 流量上升 |
| Optimal3 | 62.85 | 64.48 | +3% | CPU bound（DP 解析），tar 加速幾乎無影響 |
| Lazy2 | 62.50 | 70.27 | +12% | 輕度受益，lazy parse 仍為瓶頸 |
| Optimal | 33.73 | 35.55 | +5% | CPU bound（BVX3 DP） |
| BVX3 | 371.09 | 540.22 | +46% | tar 輸入管線加速 |
| Apple | 135.03 | 153.86 | +14% | 部分受益 |
| TLZ4 | 390.15 | 586.33 | +50% | tar 輸入管線加速 |
| ZSTD | 346.94 | 429.27 | +24% | tar 輸入管線加速 |

### llama.cpp（n=40）

| 格式 | R42 Enc MB/s | R43 Enc MB/s | 變化| 主因|
| --- | ---: | ---: | ---: | --- |
| TGZ | 39.63 | 243.69 | **+515%** | swift_tar 多核心 gzip |
| Other3 | 87.49 | 247.42 | +183% | tar 加速（llama.cpp 大量小檔，I/O 佔比更高） |
| Optimal3 | 64.07 | 88.62 | +38% | 部分受益，仍 CPU bound |
| Lazy2 | 86.62 | 190.31 | +120% | tar I/O 瓶頸解除後，lazy parse 更充分利用 CPU |
| Optimal | 48.01 | 62.11 | +29% | CPU bound（BVX3 DP） |
| BVX3 | 87.79 | 405.01 | +361% | tar 輸入管線加速 |
| Apple | 67.39 | 169.01 | +151% | tar 輸入管線加速 |
| TLZ4 | 87.17 | 363.08 | +317% | tar 輸入管線加速 |
| ZSTD | 92.37 | 451.24 | +388% | tar 輸入管線加速 |

## 3. Peak RSS（Mac only, n=40）

> TGZ 在 R43 使用 swift_tar（全檔並行讀入記憶體）；其他格式的 RSS 為 lzfse encode 管線的峰值。
...
### Peak RSS 對照：R42 vs R43（全格式，n=40）

> TGZ 在 R42（system tar）為串流模式（~4 MB），R43（swift_tar）為並行全緩衝模式（~2.9 GB）；兩者記憶體模型根本不同，不以百分比計算，以 `†` 標示。

#### claw-code（n=40）

| 格式 | R42 Enc RSS | R43 Enc RSS | Enc 變化 | R42 Dec RSS | R43 Dec RSS | Dec 變化 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
> **Enc RSS 規律**：
> - **claw-code**（少量大檔）：I/O 密集格式（Other3 +56%、BVX3 +47%）RSS 上升，因 swift_tar 多核心同時持有大檔 buffer；CPU bound 格式（Optimal3 +0.5%、Lazy2 +0.3%）幾乎不變（DP 工作集獨立於 tar 速度）。
> - **llama.cpp**（大量小檔）：多數格式 RSS 下降（Other3 −37%、Optimal3 −33%、BVX3 −34%、Optimal −24%、Apple −15%），因 swift_tar 批次打包小檔後 pipe 更連貫，減少 lzfse 的 in-flight buffer；例外：Lazy2 +74%，推測 swift_tar 更高輸入速率使 lazy parse 持有更大滑動視窗。
> - **Dec RSS** 兩輪幾乎無變化（解碼路徑不受 tar 實作影響）。

## 4. CPU Energy（Mac only, n=40）
...
### CPU Energy 對照：R42 vs R43（全格式，n=40）

#### claw-code（n=40）

| 格式 | R42 Enc J | R43 Enc J | Enc J 變化 | R42 J/TGZ | R43 J/TGZ | J/TGZ 變化 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
> **解讀**：
> - **TGZ**：R43 絕對能耗反而**下降** −33%～−39%：雖然 swift_tar 啟動多核心，但 encode 時間從 ~28s 縮短至 ~4.6s（claw-code），總能耗仍降低。
> - **Apple**：增幅最大（+136%～+159%）：R42 encode 有大量 I/O 等待，R43 全程 CPU 滿載，能耗顯著上升。
> - **Lazy2**：增幅次之（+115%～+132%）：lazy parse 在更快的 pipe 輸入下 I/O stall 消失，CPU 時間大幅增加。
> - **J/TGZ ratio** 全面上升（+96%～+291%）：TGZ 因 swift_tar gzip 並行化能耗大降成為更嚴格的基準，所有其他格式的相對比率因此走高。

## 5. Best Points（Optimal3, all n）
...
# R42-Win：other3 -optimal3 Windows round 補齊（2026-07-05）
...
> **修正記錄（2026-07-05 補測）**：`helper_windows/decode-win.bat` 的
...
## 1. Windows 實測結果（n=40 inflight，修正後 / corrected）
...
## 2a. 為什麼 Optimal3 與 Optimal 壓縮比仍差很多
...
# Pre-R42：LZFSE_Win_UI — Windows 圖形介面與打包工具鏈（2026-06-27）
...
## codec 變更：Swift 6 嚴格並行相容（純標註，零邏輯變更）
...
# R41-Mac：Tag-packed Hash Chain 導入（2026-06-22）
...
## 1a. Encode 速度 vs Windows（claw-code, n=40）
...
## 1b. Decode 速度（Mac + Win, claw-code n=40）
...
## 2. RSS 峰值（Mac only, claw-code n=40）
...
## 3. CPU Energy（Mac only, claw-code n=40）
...
# R41-Win：Windows Benchmark 結果 + Decode 驗證基礎設施（2026-06-23）
...
## 1a. Encode 速度 vs Mac（claw-code, n=40）
...
## 1b. Decode 速度 + 驗證（首輪 Win, claw-code n=40）
...
# R41-Mac-Retest：以 UI 支援代碼重測（2026-06-23）
...
## 1a. Encode 速度 vs R41-Mac 首輪（claw-code, n=40）
...
## 5. Encode 速度全覽（claw-code + llama.cpp, n=40）
...
# R41-Win Retest：雙資料集完整 Windows Benchmark（2026-06-28）
...
>
> **重要發現**：llama.cpp 資料集上，Windows LZFSE encode 速度**超越** Mac（Other3/BVX3 ≈ 1.32–1.35×），claw-code 上仍以 Mac 領先。
...
## 1b. Encode 速度 vs Mac（llama.cpp, n=40）
...
## 1c. Decode 速度（file write mode，claw-code n=40）
...
## 1d. Decode 速度（file write mode，llama.cpp n=40）
...
## Mac Encode 速度 vs R40（claw-code, n=40，Retest 最終值）
...
# R40-Mac：macOS 完整 Benchmark 結果（2026-06-22）
...
## 1b. Decode 速度（Mac only, claw-code n=40）
...
# R40-Win：Optimal 跨段 match 越界修復（2026-06-21）
...
# R40：Streaming decode 復原與 encode 管線修正（2026-06-21）
...
# R39：92221a02 encode 回退版重測（2026-06-21）
...
## Encode energy 四輪比對分析（R35 / R36 / R37
...
# 第三十七輪：R36 rep1/rep2 支配跳過同碼重測（性能收益未重現）（2026-06-20）
...
# 第三十六輪：Optimal 能耗/速度 — rep1/rep2 支配跳過（output-identical 通過）（2026-06-19）
...
# 第三十四輪：Codex #4 — tag-less length-4 補檢（2026-06-19）
...
# 第三十輪：Optimal cheap-probe gating + literal UBP DOE 結果（2026-06-16）
...
# 第二十九輪：R26①② + R28 合併 DOE 結果 + 首次 power/energy 量測（2026-06-16）
...
# 第二十六輪：BVX3 / Lazy2 CPU 單點優化（2026-06-15）
...
## 改動 2：`lzParseChain.bestMatch` 的 rep 長度快取
...
# 第二十三輪：benchmark / memProbe / trace 統整重算（2026-06-14）
...
# 修改方向建議：bvx3 家族記憶體整體解法（R23 設計筆記）
...
# 第二十二輪：Time Profiler trace 全覆蓋（2026-06-14）
...
# 第二十一輪：完整 memProbe 覆蓋與重測（2026-06-14）
...
# 第二十輪：完整 benchmark + memprobe 重新整理（2026-06-14）
...
# 第十九輪：平行編碼有界緩衝（backpressure）落地後重測（2026-06-14）
...
### 同輪內相對：Optimal / Lazy2 壓縮時間倍數（最可信）
...
# 第十七輪：熵閘調參 + 三點取樣 + 文字保護 + warm-cache（2026-06-14）
...
# 第十六輪：熵感知閘門（資料驅動 GGUF 分區）（2026-06-13）
...
# 第十四輪：Gemini 搜尋預算 + 熵代理（2026-06-13）
...
# 第十一輪：lz4bench 修復 + 解壓縮基線重建（2026-06-13）
...
# 第九輪：基線驗證 + MB/s 比較基準確立（2026-06-13）
...
# 第七輪：BT match finder 實驗（2026-06-13）
...
# 第五輪：lazy2/optimal 提速調參（2026-06-13）
...
# 第二輪：Optimal Parsing 策略（2026-06-12）
