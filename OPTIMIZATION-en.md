...
1. ** `findstr /x` is unreliable for LF-only files**: `swift_tar/version.txt` is generated by `generate_version.sh` (zsh script) with `echo`, which is an LF-only line line back; Windows `findstr /x` (full line is fully compared) expects CRLF ending, even if `findstr /n` (no `/x`) can find the content of the line correctly, `/x` still cannot be found, causing `run_round.bat -swift_tar` to print " `-swift_tar requires zlib_linkage=static in version.txt`" and suspend. Method: `run_round.bat` line 51 is changed to substring comparison without adding `/x` (key=value is the only string in the file, safe).
...
> **Goal**: Conclusion of R46-Win-Retest Points 3 and 4 point out two unresolved gaps--(a) `decode-win.bat` The TGZ null-mode is actually `tar xzf ... > nul`, `> nul` Only discard stdout, `x` It will still fall, which is not the real to-null decode; (b) benchmark Each script is called through PATH shim `tar`, there is no verification at the time of execution to really parse `swift_tar.exe` Link with its native zlib. This time, the output is modified by codex review, covering four Windows `.bat` / `.ps1` With macOS's `run_round.command` / `zshrc.sh`, **It is only an infrastructure correction, and the complete R47 benchmark has not been executed. This commit does not contain new performance data**.
...
- Export `SWIFT_TAR_BIN`, `LZFSE_REQUIRE_NATIVE_ZLIB=1` ( `0` when not enabled); add `--preserve-env=PATH,SWIFT_TAR_BIN,LZFSE_REQUIRE_NATIVE_ZLIB` when `sudo` calls `benchmark2.sh` to ensure that the native zlib flag and path are not lost in the sudo sub-itinerary.
...
| `benchmark2.sh`: Step 8/9 Restore call | Because Step 8/9 itself can elegantly handle the "no new trace" situation, there is no need to skip the whole annotation; change back to normal call, rely on the script's own exit 0 to protect the pipeline. |
